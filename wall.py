...
'''
    ...
    def sync_endpoints(self, endpoints, specific_file=None, retry_count=0):
        ...
        try:
            ...
            if not new_files:
                return
            self.log('INICIANDO ENVÍO DE REPLAY (CEL -> CEL)...', 'info')
            for file_name in sorted(new_files):
                if not file_name.endswith('.bin'):
                    continue
                self.log_detail(f'[DATO] PROCESANDO PAQUETE: {file_name[:10]}...', 'info')
                ...
                self.log_detail('[VERIFICACIÓN] VERIFICANDO REPLAY DUPLICADO...', 'info')
                ...
                try:
                    ...
                    self.log(f'[ALERTA] ERROR AL VERIFICAR DUPLICADOS: {str(e)[:50]}', 'warning')
                ...
                try:
                    ...
                    if retry_count > 0 and target_replay_files:
                        ...
                        self.log_detail('[SOBRESCRITURA] Reintento: usando el replay más reciente como base...', 'info')
                        ...
                        self.log_detail('[SOBRESCRITURA] Sobrescribiendo el replay duplicado...', 'info')
                    elif target_replay_files:
                        target_json_remote = target_replay_files[-1]
                        self.log_detail('[SOBRESCRITURA] Sobrescribiendo el replay más antiguo...', 'info')
                        target_bin_remote = target_json_remote.replace('.json', '.bin')
                except Exception as e:
                    self.log(f'[ALERTA] ERROR AL ENCONTRAR REPLAY BASE: {str(e)}', 'warning')

                if not target_bin_remote or not target_json_remote:
                    self.log('[ALERTA] No se encontró replay base para sobrescribir.', 'warning')
                    continue
                ...
                if source_data:
                    try:
                        ...
                        self.log(f'[ALERTA] ERROR EN LA TRANSFERENCIA: {str(e)}', 'warning')
                ...
                self.log_detail('[VERIFICACIÓN] ANALIZANDO SINCRONÍA...', 'info')
                ...
                if resultado is True:
                    ...
                    if retry_count < max_retries:
                        retry_count += 1
                        self.log_detail(f'[REINTENTO {retry_count}/{max_retries}] Reenviando replay...', 'info')
                        time.sleep(2)
    ...
    def manual_send_replay(self, endpoints):
        def task():
            try:
                src_ep = endpoints['source']
                tgt_ep = endpoints['target']
                self.log(f"[ENVIO] SOURCE (origen): {src_ep.get('id') or 'LOCAL (S9)'}", 'info')
                ...
                if files:
                    ...
                    if latest_file:
                        ...
                        self.log(
                            '[ERROR] NINGÚN ARCHIVO .bin ENCONTRADO EN SOURCE (origen)',
                            'error',
                        )
                else:
    ...
    def burla_logs_action(self, source_id=None, target_id=None):
        ...
        def _detect_package(serial):
            for package in PACKAGES:
                ...
                if result == "1":
                    return package
            raise BurlaLogsError(
                f"ffrtc_log.txt no encontrado en el dispositivo {serial}."
            )
        ...
        def _merge_ffrtc(target_content, source_content):
            ...
            if not target_blocks:
                raise BurlaLogsError(
                    "Fecha y hora del primer log del celular TARGET no encontradas."
                )
            ...
            if not target_identity_blocks:
                raise BurlaLogsError(
                    "Bloque de identificación del celular TARGET no encontrado."
                )
            if not source_log_blocks:
                raise BurlaLogsError(
                    "No hay logs nuevos en SOURCE después del inicio "
                    f"del FFRTC de TARGET ({first_target_timestamp})."
                )
        ...
        def _remote_identity(ep, remote_path):
            ...
            if not identity:
                raise BurlaLogsError(
                    "No fue posible leer inode, UID y GID del FFRTC."
                )
            return identity
        ...
        def _detect_package_ep(ep):
            ...
            raise BurlaLogsError(
                f"ffrtc_log.txt no encontrado en el endpoint "
        ...
        def _burlar_logs(target_ep, source_ep):
            ...
            self.log_detail(
                f'[BURLA] [LOGS] SOURCE (logs de partidas): {source_package.split(".")[-1]} '
                f'| TARGET (identidad): {target_package.split(".")[-1]}',
                'info')
            ...
            with tempfile.TemporaryDirectory(prefix="burla_ffrtc_") as temp_dir:
                ...
                self.log_detail('[BURLA] [LOGS] DESCARGANDO LOG DE TARGET (identidad)...', 'info')
                _pull_file(target_ep, target_remote, target_local)
                self.log_detail('[BURLA] [LOGS] DESCARGANDO LOG DE SOURCE (contenido de partidas)...', 'info')
                ...
                if not target_content:
                    raise BurlaLogsError("FFRTC de TARGET está vacío.")
                if not source_content:
                    raise BurlaLogsError("FFRTC de SOURCE está vacío.")

                self.log_detail('[BURLA] [LOGS] MEZCLANDO BLOQUES (identidad TARGET + logs SOURCE)...', 'info')
                ...
                try:
                    ...
                    self.log_detail('[BURLA] [LOGS] SOBRESCRIBIENDO FFRTC EN TARGET...', 'info')
                    overwrite_command = (
                ...
                if identity_after != identity_before:
                    raise BurlaLogsError(
                        "Inode, UID o GID cambiaron durante la sobrescritura.\n"
                        f"Antes:  {identity_before}\n"
                        f"Después: {identity_after}"
                    )
                self.log(f'[BURLA] [OK] Inode:UID:GID preservados: {identity_after}', 'info')
                self.log('FFRTC BURLADA CON ÉXITO', 'success')

        def task():
            try:
                self.log('[BURLA] BURLANDO FFRTC...', 'secure')
                src_ep = endpoints['source']
                tgt_ep = endpoints['target']
                self.log(
                    f"[FFRTC] SOURCE (logs de partidas): {src_ep.get('id') or 'LOCAL (S9)'}",
                    'info')
                self.log(
                    f"[FFRTC] TARGET (destino/identidad): {tgt_ep.get('id') or 'LOCAL (S9)'}",
                    'info')
    ...
    def list_devices(self):
        ...
        self.log(f'{BOLD}Endpoints disponibles:{RESET}', 'info')
        if has_local:
            self.log(f'  {GREEN}[LOCAL]{RESET} S9 (este celular, acceso directo via /sdcard)', 'info')
        if not devices:
        ...
        if not devices and not has_local:
            self.log('Ningún endpoint disponible. Verifica ADB o permisos de /sdcard.', 'warning')
            return
        self.log(
...
def connect_device():
    ...
    ip = input('Introduce la IP del dispositivo: ').strip()
    port = ask_port('Introduce el PUERTO de conexión (ej.: 5555): ')
...
def status():
    ...
    if code != 0:
        ...
        print(f'\n{RED}No se pudo consultar ADB.{RESET}')
        pause()
        return
...
def list_read_only():
    ...
    if not serials:
        print(f'{YELLOW}Ningún S21 autorizado por ADB.{RESET}')
        pause()
        return
...
def update_termux_environment():
    ...
    print(f'{BLUE}Actualizando paquetes...{RESET}')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Tu CONFIGURACIÓN (Termux S9 emparejado con S21 mediante ADB Wi-Fi, 1 dispositivo en adb devices):
  S9 = endpoint LOCAL (accede a /sdcard directamente sin ADB)
  S21 = endpoint REMOTO (accede mediante ADB, la IP:puerto que aparece en `adb devices`)

Usa --source-role para indicar CUÁL DE LOS DOS TIENE EL REPLAY A ENVIAR (origen = SOURCE).
El otro automáticamente se convierte en TARGET (recibe el replay / recibe la identidad de la burla FFRTC).
...
2) S9 tiene el replay (SOURCE=local), quiere enviar a S21 (TARGET=remote):
   python3 wall_cli.py send \\
       --source-role local \\
       --local-version max --target-version max

3) S21 tiene el replay (SOURCE=remote), quiere enviar a S9 (TARGET=local):
   python3 wall_cli.py send \\
       --source-role remote \\
       --source-version max --local-version normal

4) Ver último replay + veredicto en S21 (TARGET = remote):
   python3 wall_cli.py check-status --source-role local --target-version max
   (el objetivo es el TARGET, es decir, el opuesto a source-role)

5) Burla FFRTC: usa el LOG del S21 (SOURCE=remote) como contenido de partidas,
   aplica la IDENTIDAD del S9 (TARGET=local):
   python3 wall_cli.py burla-ffrtc \\
       --source-role remote \\
       --source-version max --local-version max

6) Modo detallado:
   RIBEIRO_VERBOSE=1 python3 wall_cli.py send --source-role local
        """)
    parser.add_argument('--mode', choices=['auto', 'mixed', 'adb_adb'],
                        default='auto',
                        help='auto = mezcla LOCAL+ADB si hay 1 dispositivo; adb_adb = 2 celulares via ADB')
    parser.add_argument('--local-version', choices=['normal', 'max'],
                        default='normal',
                        help='Versión de Free Fire en S9 LOCAL (default: normal)')
    parser.add_argument('--target-version', choices=['normal', 'max'],
                        default='normal',
                        help='Versión de FF en endpoint REMOTO ADB cuando es TARGET (destino)')
    parser.add_argument('--source-version', choices=['normal', 'max'],
                        default='normal',
                        help='Versión de FF en endpoint REMOTO ADB cuando es SOURCE (origen)')
    parser.add_argument('--source-role', choices=['local', 'remote'],
                        default='remote',
                        help='¿Qué celular es SOURCE (origen de replays/logs)? local = S9, remote = celular via ADB (S21). (default: remote)')
    parser.add_argument('--remote-id', default=None,
                        help='Forzar qué serial ADB es el endpoint remoto (solo si hay más de 1 en adb devices)')

    subparsers = parser.add_subparsers(dest='command', required=True)
    subparsers.add_parser('devices', help='Listar endpoints disponibles (LOCAL S9 + ADB remotos)')
    subparsers.add_parser('send', help='Enviar replay MÁS RECIENTE SOURCE -> TARGET (preserva inode + timestamps)')
    subparsers.add_parser('check-status', help='Verificar último replay en TARGET (stat completo + veredicto)')
    subparsers.add_parser('check-ffrtc', help='Verificar stat del ffrtc_log.txt en TARGET')
    subparsers.add_parser('burla-ffrtc', help='Burla FFRTC (logs SOURCE + identidad TARGET, preserva inode)')
