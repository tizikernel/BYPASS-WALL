...
class RibeiroCLI:
    ...
    def log_detail(self, msg, tag='info'):
        verbose = os.environ.get('WALL_VERBOSE', '0') == '1'
        if verbose:
            self.log(msg, tag)
    ...
    def _build_replay_commit_command(
        ...
        if not target_suffix or any(ch not in allowed for ch in target_suffix):
            raise ValueError('Sufijo de replay no válido para la transferencia.')
...
echo 'Nao foi possivel convergir os timestamps no mesmo segundo.' >&2
exit 32
'''
    ...
    def sync_endpoints(self, endpoints, specific_file=None, retry_count=0):
        ...
        try:
            ...
            if not new_files:
                return
            self.log('🚀 INICIANDO ENVIO DO REPLAY (CEL -> CEL)...', 'info')
            for file_name in sorted(new_files):
                if not file_name.endswith('.bin'):
                    continue
                self.log_detail(f'📦 [DADO] PROCESSANDO PACOTE: {file_name[:10]}...\n', 'info')
                ...
                self.log_detail('🔍 [VERIFICAÇÃO] VERIFICANDO REPLAY DUPLICADO...\n', 'info')
                ...
                try:
                    ...
                    self.log(f'⚠️ [ALERTA] ERRO AO VERIFICAR DUPLICATAS: {str(e)[:50]}\n', 'warning')
                ...
                try:
                    ...
                    if retry_count > 0 and target_replay_files:
                        ...
                        self.log_detail('🗂️ [SOBRESCRITA] Retentativa: usando o replay mais recente como base...\n', 'info')
                        ...
                        self.log_detail('🔄 [SOBRESCRITA] Sobrescrevendo o replay duplicado...\n', 'info')
                    elif target_replay_files:
                        target_json_remote = target_replay_files[-1]
                        self.log_detail('🗂️ [SOBRESCRITA] Sobrescrevendo o replay mais antigo...\n', 'info')
                        target_bin_remote = target_json_remote.replace('.json', '.bin')
                except Exception as e:
                    self.log(f'⚠️ [ALERTA] ERRO AO ENCONTRAR REPLAY BASE: {str(e)}\n', 'warning')

                if not target_bin_remote or not target_json_remote:
                    self.log('⚠️ [ALERTA] Nenhum replay base encontrado para sobrescrever.\n', 'warning')
                    continue
                ...
                if source_data:
                    try:
                        ...
                        self.log(f'⚠️ [ALERTA] ERROR EN LA TRANSFERENCIA: {str(e)}\n', 'warning')
                ...
                self.log('✅ REPLAY ENVIADO CORRECTAMENTE', 'success')
                time.sleep(0.15)

                self.log_detail('🔍 [VERIFICAÇÃO] ANALISANDO SINCRONIA...', 'info')
                resultado = self._verificar_veredito_ep(tgt_ep, replay_path, bin_path=new_bin_remote)
                if resultado is True:
                    self.log('✅ REPLAY 100% SINCRONIZADO', 'success')
                    self.burla_logs_endpoints(endpoints)
                elif resultado is False:
                    self.log('❌ REPLAY ALTERADO', 'error')
                    max_retries = 3
                    if retry_count < max_retries:
                        retry_count += 1
                        self.log_detail(f'🔄 [RETENTATIVA {retry_count}/{max_retries}] Reenviando replay...', 'info')
                        time.sleep(2)
                        self.sync_endpoints(endpoints, specific_file=file_name, retry_count=retry_count)
                    else:
                        self.log(f'❌ [FALHA] Máximo de reintentos ({max_retries}) atingido.', 'error')
        except Exception as e:
    ...
    def check_target_endpoint(self, endpoints):
        ...
        try:
            ...
            for label, full_path in (
                    (None, bin_path), (None, json_path), ('Pasta MReplays', replay_path)):
                if label:
                    self.log(label, 'info')
            ...
            if resultado is True:
                self.log('✅ VEREDICTO: REPLAY 100% ORIGINAL', 'verdict_original')
            elif resultado is False:
                self.log('❌ VEREDICTO: REPLAY ALTERADO / PASADO', 'error')
            else:
    ...
    def verify_ffrtc_endpoint(self, endpoints, max_only=False):
        ...
        def task():
            try:
                self.log(f"[FFRTC] Dispositivo alvo: {tgt_ep.get('id') or 'LOCAL (S9)'}", 'info')
                candidates = [
                ...
                for label, path in candidates:
                    try:
                        ...
                        continue
                if not found_any:
                    scope = 'MAX' if max_only else 'MAX e NORMAL'
                    self.log(
                        f'[AVISO] [FFRTC] Nenhum ffrtc_log.txt encontrado ({scope}).',
                        'warning',
                    )
                    return
                self.log('', 'info')
                ...
                self.log(f'[ERROR] [FFRTC] FALHA: {str(e)[:120]}', 'error')
                self.log(f'[DETALHES] {traceback.format_exc()[:500]}', 'warning')
    ...
    def manual_send_replay(self, endpoints):
        def task():
            try:
                src_ep = endpoints['source']
                tgt_ep = endpoints['target']
                self.log(f"[ENVIO] SOURCE (fonte): {src_ep.get('id') or 'LOCAL (S9)'}", 'info')
                self.log(f"[ENVIO] TARGET (destino): {tgt_ep.get('id') or 'LOCAL (S9)'}", 'info')
                    ...
                    if latest_file:
                        ...
                        if src_ep.get('kind') == 'local':
                            ...
                            if not os.path.isfile(json_path):
                                ...
                                return
                        self.log(
                            f'[ENVIO] Último replay seleccionado: {latest_file}',
                            'success',
                        )
                        self.log(
                            f'[ENVIO] JSON correspondiente: {matching_json}',
                            'info',
                        )
                        self.sync_endpoints(
                        ...
                        self.log(
                            '[ERROR] NENHUM ARQUIVO .bin ENCONTRADO NO SOURCE (fonte)',
                            'error',
                        )
                else:
                    self.log('[ERROR] NO SE ENCONTRÓ NINGÚN REPLAY EN SOURCE (origen)', 'error')
            except Exception as e:
                self.log(f'[ALERTA] ERROR AL BUSCAR EL REPLAY: {str(e)} ', 'error')
                self.log(f'[DETALHES] {traceback.format_exc()}', 'warning')
        t = threading.Thread(target=task, daemon=True)
        t.start()
        t.join()
    ...
    def burla_logs_action(self, source_id=None, target_id=None):
        ...
        def _run_adb(*arguments, serial=None, check=True):
            ...
            if check and result.returncode != 0:
                ...
                raise BurlaLogsError(
                    f"ADB falhou ({result.returncode}): {' '.join(command)}\n{message[:300]}"
                )
            return result
        ...
        def _detect_package(serial):
            for package in PACKAGES:
                ...
                if result == "1":
                    return package
            raise BurlaLogsError(
                f"ffrtc_log.txt nao encontrado no dispositivo {serial}."
            )
        ...
        def _merge_ffrtc(target_content, source_content):
            ...
            if not target_blocks:
                raise BurlaLogsError(
                    "Data e hora da primeira log do celular TARGET nao encontradas."
                )
            ...
            if not target_identity_blocks:
                raise BurlaLogsError(
                    "Bloco de identificacao do celular TARGET nao encontrado."
                )
            if not source_log_blocks:
                raise BurlaLogsError(
                    "Nao ha logs novas no SOURCE depois do inicio "
                    f"do FFRTC do TARGET ({first_target_timestamp})."
                )
        ...
        def _remote_identity(ep, remote_path):
            ...
            if not identity:
                raise BurlaLogsError(
                    "Nao foi possivel ler inode, UID e GID do FFRTC."
                )
            return identity
        ...
        def _detect_package_ep(ep):
            ...
            raise BurlaLogsError(
                f"ffrtc_log.txt nao encontrado no endpoint "
        ...
        def _burlar_logs(target_ep, source_ep):
            ...
            self.log_detail(
                f'[BURLA] [LOGS] SOURCE (fonte partidas): {source_package.split(".")[-1]} '
                f'| TARGET (identidade): {target_package.split(".")[-1]}',
                'info')
            ...
            with tempfile.TemporaryDirectory(prefix="burla_ffrtc_") as temp_dir:
                ...
                self.log_detail('[BURLA] [LOGS] BAIXANDO LOG DO TARGET (identidade)...', 'info')
                _pull_file(target_ep, target_remote, target_local)
                self.log_detail('[BURLA] [LOGS] BAIXANDO LOG DO SOURCE (conteudo das partidas)...', 'info')
                ...
                if not target_content:
                    raise BurlaLogsError("FFRTC do TARGET esta vazio.")
                if not source_content:
                    raise BurlaLogsError("FFRTC do SOURCE esta vazio.")

                self.log_detail('[BURLA] [LOGS] MESCLANDO BLOCOS (identidade TARGET + logs SOURCE)...', 'info')
                ...
                try:
                    self.log_detail('[BURLA] [LOGS] ENVIANDO LOG MODIFICADO (TEMP)...', 'info')
                    _push_file(target_ep, modified_local, remote_temp)
                    remote_temp_created = True

                    self.log_detail('[BURLA] [LOGS] SOBRESCREVENDO FFRTC NO TARGET...', 'info')
                    overwrite_command = (
                ...
                if identity_after != identity_before:
                    raise BurlaLogsError(
                        "Inode, UID ou GID mudou durante a sobrescrita.\n"
                        f"Antes:  {identity_before}\n"
                        f"Depois: {identity_after}"
                    )
                self.log(f'[BURLA] [OK] Inode:UID:GID preservados: {identity_after}', 'info')
                self.log('✅ FFRTC BURLADA COM SUCESSO', 'success')

        def task():
            try:
                self.log('🔒 BURLANDO FFRTC...', 'secure')
                src_ep = endpoints['source']
                tgt_ep = endpoints['target']
                self.log(
                    f"[FFRTC] SOURCE (fonte partidas): {src_ep.get('id') or 'LOCAL (S9)'}",
                    'info')
                self.log(
                    f"[FFRTC] TARGET (destino/identidade): {tgt_ep.get('id') or 'LOCAL (S9)'}",
                    'info')
                _burlar_logs(tgt_ep, src_ep)
            except BurlaLogsError as error:
                self.log(f'[ERROR] [BURLA] {error}', 'error')
            except Exception as e:
                self.log(f'[ERROR] [BURLA] ERRO: {str(e)}', 'error')
                self.log(f'[DETALHES] {traceback.format_exc()[:500]}', 'warning')
        t = threading.Thread(target=task, daemon=True)
        t.start()
        t.join()
    ...
    def list_devices(self):
        ...
        self.log(f'{BOLD}Endpoints disponiveis:{RESET}', 'info')
        if has_local:
            self.log(f'  {GREEN}[LOCAL]{RESET} S9 (este celular, acesso direto via /sdcard)', 'info')
        if not devices:
            self.log('  (nenhum dispositivo ADB adicional detectado)', 'warning')
        for i, d in enumerate(devices):
            cor = BLUE
            self.log(f'  {cor}[ADB #{i}]{RESET} {d}', 'info')
        if not devices and not has_local:
            self.log('Nenhum endpoint disponivel. Verifique ADB ou permissoes /sdcard.', 'warning')
            return
        self.log(
            f'{CYAN}Use --source-role=local (S9 como fonte) ou '
            f'--source-role=remote (celular ADB como fonte).{RESET}',
            'info')
...
def run_adb(args, timeout=30):
    try:
        ...
        return 127, 'ADB não encontrado. Instale com: pkg install android-tools'
    except subprocess.TimeoutExpired:
        return 124, 'Tempo esgotado aguardando o ADB.'
...
def header():
    print()
    print(f'{WHITE}{BOLD}✦  WALL ANDROID PRIVATE  ✦{RESET}')
...
def connect_device():
    ...
    ip = input('Introduce la IP del dispositivo: ').strip()
    port = ask_port('Digite a PORTA de conexão (ex.: 5555): ')
...
def list_read_only():
    clear()
    header()
    print(f'{CYAN}{BOLD}REPLAYS DEL S21 — FREE FIRE MAX{RESET}\n')
    ...
    print(f'{BLUE}Pasta: {replay_path}{RESET}\n')
    script = (
        f"find {shlex.quote(replay_path)} -maxdepth 1 -type f "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='WALL CLI - Transferencia de replay OPSEC (S9 LOCAL + celular ADB remoto)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Tu CONFIGURACIÓN (Termux do S9 pareado com S21 via ADB Wi-Fi, 1 dispositivo no adb devices):
  S9 = LOCAL endpoint  (acessa /sdcard diretamente sem ADB)
  S21 = REMOTE endpoint (acessa via ADB, o IP:porta que aparece no `adb devices`)

Use --source-role para dizer QUAL DOS DOIS TEM O REPLAY A SER ENVIADO (fonte = SOURCE).
O outro automaticamente vira TARGET (recebe o replay / recebe a identidade da burla FFRTC).

======== EXEMPLOS PARA SEU CENÁRIO ========

1) Listar endpoints
   python3 wall_cli.py devices

2) S9 tem o replay (SOURCE=local), quer enviar pro S21 (TARGET=remote):
   python3 wall_cli.py send \\
       --source-role local \\
       --local-version max --target-version max

3) S21 tem o replay (SOURCE=remote), quer enviar pro S9 (TARGET=local):
   python3 wall_cli.py send \\
       --source-role remote \\
       --source-version max --local-version normal

4) Ver ultimo replay + veredito no S21 (TARGET = remote):
   python3 wall_cli.py check-status --source-role local --target-version max
   (alvo eh o TARGET, ou seja, o oposto do source-role)

5) Burla FFRTC: usa o LOG DO S21 (SOURCE=remote) como conteudo de partidas,
   aplica a IDENTIDADE do S9 (TARGET=local):
   python3 wall_cli.py burla-ffrtc \\
       --source-role remote \\
       --source-version max --local-version max

6) Modo detalhado:
   WALL_VERBOSE=1 python3 wall_cli.py send --source-role local
        """
    )
    parser.add_argument('--mode', choices=['auto', 'mixed', 'adb_adb'],
                        default='auto',
                        help='auto = mistura LOCAL+ADB se tiver so 1 device; adb_adb = 2 celulares via ADB')
    parser.add_argument('--local-version', choices=['normal', 'max'],
                        default='normal',
                        help='Versão Free Fire no S9 LOCAL (default: normal)')
    parser.add_argument('--target-version', choices=['normal', 'max'],
                        default='normal',
                        help='Versão FF no endpoint REMOTE ADB quando ele é o TARGET (destino)')
    parser.add_argument('--source-version', choices=['normal', 'max'],
                        default='normal',
                        help='Versão FF no endpoint REMOTE ADB quando ele é o SOURCE (fonte)')
    parser.add_argument('--source-role', choices=['local', 'remote'],
                        default='remote',
                        help='Qual celular eh o SOURCE (fonte dos replays/logs)? local = S9, remote = celular via ADB (S21). (default: remote)')
    parser.add_argument('--remote-id', default=None,
                        help='Força qual serial ADB é o endpoint remoto (só se tiver mais de 1 no adb devices)')

    subparsers = parser.add_subparsers(dest='command', required=True)
    subparsers.add_parser('devices', help='Lista endpoints disponiveis (LOCAL S9 + ADB remotos)')
    subparsers.add_parser('send', help='Envia replay MAIS RECENTE SOURCE -> TARGET (preserva inode + timestamps)')
    subparsers.add_parser('check-status', help='Verifica ultimo replay no TARGET (stat completo + veredito)')
    subparsers.add_parser('check-ffrtc', help='Verifica stat do ffrtc_log.txt no TARGET')
    subparsers.add_parser('burla-ffrtc', help='Burla FFRTC (logs SOURCE + identidade TARGET, preserva inode)')

    args = parser.parse_args()

    if not is_adb_available():
        print(f"{RED}{BOLD}[ERROR]{RESET} ADB não encontrado. "
              f"No Termux rode: pkg install android-tools")
        sys.exit(1)
    ...
    if args.command == 'send':
        ...
        cli.burla_logs_action(endpoints)
    else:
        parser.print_help()
        sys.exit(1)
